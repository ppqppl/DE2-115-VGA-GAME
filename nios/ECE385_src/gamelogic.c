...
void game_init_double() {
	...
	sprites_init(VGA_SPRITE_PLANE_2p);
	sprites_init(VGA_SPRITE_BULLET_2p);
//	printf("Wait for keyboard ready\n");
//	while(!keycode_comm->keyboard_present);
//	printf("Keyboard ready\n");
...
//			printf("ERR sprites_get\n");
	...
	game_state = IN_GAME_double;
}

void game_loop_double() {
	if(game_state != IN_GAME_double) return;

	if(esc_pressed()) {
		// Quit game immediately
		game_state = GAME_OVER_double;
		id_p--;
		return;
	}
	...
	if((!io_vga_sync_prev) && (*io_vga_sync)) {
		...
		if(prev_hp != (player_plane_info->hp + player_plane_info1->hp)  || prev_score != player_score) {
			...
			hp_all = 6 - (player_plane_info->hp + player_plane_info1->hp);
			printf("H1:    %d    H2:    %d\n",player_plane_info->hp,player_plane_info1->hp);
...
//					printf("ERR sprites_load_data\n");
		...
		if(player_plane_info1->type == 0) {
			...
			if(frame_count % PLAYER_PLANE_IMG_UPDATE_INTERVAL == 0) {
				const sprite_data_t* plane_data1 = plane_player_seq + (frame_count / PLAYER_PLANE_IMG_UPDATE_INTERVAL) % PLANE_PLAYER_SEQ_LEN;
				// Update player plane image every few frames
				if(255 == sprites_load_data(VGA_SPRITE_PLANE_2p, player_plane_id1,
						plane_data1->data,
						plane_data1->width * plane_data1->height)) {
//					printf("ERR sprites_load_data\n");
				}
			}
		} else {
		...
		// 2p
//		sprites_tick_2p(VGA_SPRITE_PLANE_2p);
//		sprites_tick_2p(VGA_SPRITE_BULLET_2p);
...
//		if(i == 0) *io_hex = keycode;
	...
	// Prevent plane from flying out of screen
	sprites_limit_speed_2p(VGA_SPRITE_PLANE_2p, player_plane_id);

	if(frame_count % PLAYER_BULLET_INTERVAL == 0) {
		for(int i = 0; i < 3; i++) {
			int id;
			if(255 == (id = sprites_allocate(VGA_SPRITE_BULLET_2p))) {
//				printf("ERR sprites_allocate\n");
				break;
			}
			volatile vga_sprite_info_t* sprite_info = sprites_get(VGA_SPRITE_BULLET_2p, id);
			if(NULL == sprite_info) {
//				printf("ERR sprites_get\n");
				break;
			}
...
//		}
//
//		if(255 == (id1 = sprites_allocate(VGA_SPRITE_PLANE_2p))) {
////			printf("ERR sprites_allocate\n");
//			return;
//		}
//		if(255 == sprites_load_data(VGA_SPRITE_PLANE_2p, id1, resource, width * height)) {
////			printf("ERR sprites_load_data\n");
//			return;
//		}
//		volatile vga_sprite_info_t* sprite_info1 = sprites_get(VGA_SPRITE_PLANE_2p, id1);
//		if(NULL == sprite_info1) {
////			printf("ERR sprites_get\n");
//			return;
//		}
//
...
//
//
//			sprite_info1->physical->x = ((-width) << VGA_SPRITE_HW_SHIFT_BITS) + 1;
//			sprite_info1->physical->y = generate_pos << VGA_SPRITE_HW_SHIFT_BITS;
//			sprite_info1->ax = rng_generate() % (2 * ENEMY_PLANE_AX) - ENEMY_PLANE_AX;
//			sprite_info1->ay = rng_generate() % (2 * ENEMY_PLANE_AY) - ENEMY_PLANE_AY;
//
//			sprite_info1->vx = rng_generate() % ENEMY_PLANE_VX;
//			if(sprite_info1->ax < 0) {
//				sprite_info1->vx_min = ENEMY_PLANE_VX / 2;
//			} else {
//				sprite_info1->vx_min = 0;
//			}
//			sprite_info1->vx_max = ENEMY_PLANE_VX * 2;
//			sprite_info1->vy = rng_generate() % ENEMY_PLANE_VY;
//			if(generate_pos < VGA_HEIGHT / 4) {
//				sprite_info1->vy_min = 0;
//			} else {
//				sprite_info1->vy_min = -ENEMY_PLANE_VY / 4;
//			}
//			sprite_info1->vy_max = ENEMY_PLANE_VY * 2;
//
//		} else if(generate_pos < VGA_HEIGHT) {
//			// Right top side of screen
//			sprite_info->physical->x = VGA_WIDTH << VGA_SPRITE_HW_SHIFT_BITS;
...
//
//
//			sprite_info1->physical->x = VGA_WIDTH << VGA_SPRITE_HW_SHIFT_BITS;
//			sprite_info1->physical->y = (generate_pos - VGA_HEIGHT / 2) << VGA_SPRITE_HW_SHIFT_BITS;
//			sprite_info1->ax = rng_generate() % (2 * ENEMY_PLANE_AX) - ENEMY_PLANE_AX;
//			sprite_info1->ay = rng_generate() % (2 * ENEMY_PLANE_AY) - ENEMY_PLANE_AY;
//
//			sprite_info1->vx = -(rng_generate() % ENEMY_PLANE_VX);
...
//
//
//			sprite_info1->physical->x = (generate_pos - VGA_HEIGHT) << VGA_SPRITE_HW_SHIFT_BITS;
//			sprite_info1->physical->y = 0;
//			sprite_info1->vx = rng_generate() % (2 * ENEMY_PLANE_VX) - ENEMY_PLANE_VX;
//			if(generate_pos - VGA_HEIGHT < VGA_WIDTH / 4) {
//				sprite_info1->vx_min = 0;
//			} else {
//				sprite_info1->vx_min = -ENEMY_PLANE_VX;
//			}
//			if(generate_pos - VGA_HEIGHT > VGA_WIDTH * 3 / 4) {
//				sprite_info1->vx_max = 0;
//			} else {
//				sprite_info1->vx_max = ENEMY_PLANE_VX;
//			}
//			// BE CAREFUL WHEN USING GAME_DIFFICULTY!!! See definition
//			sprite_info1->vx_max = ENEMY_PLANE_VX * GAME_DIFFICULTY;
//			sprite_info1->vy = rng_generate() % ENEMY_PLANE_VY;
//			sprite_info1->vy_min = ENEMY_PLANE_VY * GAME_DIFFICULTY / 2;
//			sprite_info1->vy_max = ENEMY_PLANE_VY * GAME_DIFFICULTY * 4;
//			sprite_info1->ax = rng_generate() % (2 * ENEMY_PLANE_AX) - ENEMY_PLANE_AX;
//			sprite_info1->ay = rng_generate() % (2 * ENEMY_PLANE_AY) - ENEMY_PLANE_AY;
//		}
//		sprite_info->type = 1;
...
//
//		sprite_info1->type = 1;
//		sprite_info1->hp = ENEMY_PLANE_HP * GAME_DIFFICULTY;
//		sprite_info1->bullet_color = enemies[res_id].bullet_color;
//
//		sprite_info1->physical->width = width;
//		sprite_info1->physical->height = height;
//
...
//				printf("ERR sprites_get\n");
	...
	vga_entity_manage_t* planes1 = VGA_SPRITE_PLANE_2p;
	for(int i = 0; i < planes1->max_size; i++) {
		volatile vga_sprite_info_t* plane_info1 = planes1->info_arr + i;
		if(!plane_info1->used) continue;
		if(plane_info1->type != 1) continue;

		int enemy_bullet_interval = ENEMY_BULLET_INTERVAL * GAME_DIFFICULTY_INV;
		if((frame_count - plane_info1->frame_created) % enemy_bullet_interval == 0) {
			int id;
			if(255 == (id = sprites_allocate(VGA_SPRITE_BULLET_2p))) {
//				printf("ERR sprites_allocate\n");
				break;
			}
			volatile vga_sprite_info_t* sprite_info1 = sprites_get(VGA_SPRITE_BULLET_2p, id);
			if(NULL == sprite_info1) {
//				printf("ERR sprites_get\n");
				break;
			}
			sprite_info1->physical->x = plane_info1->physical->x
					+ (plane_info1->physical->width << VGA_SPRITE_HW_SHIFT_BITS) / 2;
			sprite_info1->physical->y = plane_info1->physical->y
					+ (plane_info1->physical->height << VGA_SPRITE_HW_SHIFT_BITS);
			sprite_info1->physical->bullet_radius = ENEMY_BULLET_RADIUS;
			sprite_info1->physical->bullet_color = plane_info1->bullet_color;
			sprite_info1->type = 1;
			sprite_info1->vx = plane_info1->vx + ENEMY_BULLET_VX * GAME_DIFFICULTY;
			sprite_info1->vy = plane_info1->vy + ENEMY_BULLET_VY * GAME_DIFFICULTY;
			sprite_info1->vx_max = ENEMY_BULLET_VMAX * GAME_DIFFICULTY;
			sprite_info1->vx_min = -(ENEMY_BULLET_VMAX * GAME_DIFFICULTY);
			sprite_info1->vy_max = ENEMY_BULLET_VMAX * GAME_DIFFICULTY;
			sprite_info1->vy_min = -(ENEMY_BULLET_VMAX * GAME_DIFFICULTY);
			sprite_info1->ax = ENEMY_BULLET_AX;
			sprite_info1->ay = ENEMY_BULLET_AY;
