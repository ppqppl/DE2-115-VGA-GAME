...
//	printf("main_menu_init\n");
	*io_vga_background_offset = 0;
	...
	vga_statusbar_string(0, (uint8_t*) "ppqppl的雷电游戏  潘星宇 & 丁欣茹 & 吴淑江 & 陈渝 & 李长鹏");
...
//	vga_string_transparent(340, 440, (uint8_t*) "积分榜");
	...
	vga_string_transparent(160, 440, (uint8_t*) "单人游戏");
	vga_string_transparent(520, 440, (uint8_t*) "双人游戏");
	vga_string_transparent(340, 440, (uint8_t*) "积分榜");
...
void main_menu_select_option(main_menu_options option) {
	...
	case MENU_SCOREBOARD:
		...
		main_menu_pointer->physical->y = 448 << VGA_SPRITE_HW_SHIFT_BITS;
		break;
	case MENU_DOUBLEPLAY:
		main_menu_pointer->physical->x = 510 << VGA_SPRITE_HW_SHIFT_BITS;
		main_menu_pointer->physical->y = 448 << VGA_SPRITE_HW_SHIFT_BITS;
		break;
	}
	...
	if(keycode != prev_keycode) {
		...
		case 0x4F:	// Right
			if(menu_current_option == MENU_PLAY) {
				...
				main_menu_select_option(MENU_DOUBLEPLAY);
			}
			break;
		case 0x04:	// A
		case 0x50:	// Left
			...
			else if(menu_current_option == MENU_DOUBLEPLAY){
				main_menu_select_option(MENU_SCOREBOARD);
			}
			break;
		case 0x28:  // Enter
			if(menu_current_option == MENU_PLAY) {
				...
				sprites_init(VGA_SPRITE_BULLET);
				game_state = PREPARE_GAME_DOUBLE;
			}
			break;
		}
	}
